...
# select nodes for article headings
...
```{r extract tables}
# extract the Timeline table using bracket notation
# extract the Timeline table using magrittr::extract2() notation
```
...
# extract table and save as data.frame or as tibble
```
...
# replace "[", "]", "(", and ")" with "" in the column "Country

# remove all variations of "(Today: [any text here])" (regex:  " \\(Today: .*?\\)"

```

**Hints**
- Use the syntax `dplyr::mutate(Country = stringr::str_remove(Country, "TO_BE_REMOVED"))`
